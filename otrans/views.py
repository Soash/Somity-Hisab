...
def general_expense(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            UserLog.objects.create(processed_by=request.user,logs_action='Add General Expense',description=f'Category: {expense.voucher_category}, TrxID: {expense.VoucherID}')
            UserLog.objects.create(processed_by=request.user,action=f'{expense.voucher_category}',amount=expense.Amount, trx=True)
...
def general_income(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            UserLog.objects.create(processed_by=request.user,logs_action='Add General Expense',description=f'Category: {data.voucher_category}, TrxID: {data.VoucherID}')
            UserLog.objects.create(processed_by=request.user,action=f'{data.voucher_category}',amount=data.Amount)
