    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                # msg = f'প্রিয় স্যার, লোন অ্যাকাউন্ট তৈরি করা হয়েছে। ঋণের পরিমাণ Tk. {data.loan_amount}, কিস্তি {data.number_of_installments}, প্রতি কিস্তির পরিমাণ Tk. {data.installment_amount}'
                # send_sms(customer.mobile_number, msg, 'Add Loan AC', request.user)
...
def loan_collection(request, loan_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            balance = loan.paid_amount + data.Amount

            installments = InstallmentSchedule.objects.filter(loan=loan, installment_status='---').order_by('due_date')
            ...
            principal = data.Amount/(1+(loan.profit_percent/100))
            profit = data.Amount - principal
            ...
            UserLog.objects.create(processed_by=request.user,logs_action='Installment Collection',description=f'Account Number: {loan.customer.account_no}, TrxID: {data.VoucherID}')
            UserLog.objects.create(processed_by=request.user,action=f'Installment Principal {loan.customer.customer_type}',amount=principal, customer=loan.customer)
            UserLog.objects.create(processed_by=request.user,action=f'Installment Profit {loan.customer.customer_type}',amount=profit, customer=loan.customer)
